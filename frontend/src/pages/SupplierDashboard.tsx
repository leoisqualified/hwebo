...
export default function SupplierDashboard() {
  const [activeTab, setActiveTab] = useState("availableBids");
    ...
    <div className="flex min-h-screen bg-[#F3F4F6]">
      ...
      >
        ...
        <nav className="flex flex-col space-y-2">
          <motion.button
            whileHover={{ x: 5 }}
            onClick={() => setActiveTab("availableBids")}
            className={`flex items-center space-x-3 p-3 rounded-lg transition-colors ${
              activeTab === "availableBids"
                ? "bg-[#059669]"
                : "hover:bg-[#1E40AF]"
            }`}
          >
          ...
          <motion.button
            ...
            className={`flex items-center space-x-3 p-3 rounded-lg transition-colors ${
              activeTab === "myOffers" ? "bg-[#059669]" : "hover:bg-[#1E40AF]"
            }`}
          >
            <svg
              className="w-5 h-5"
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 min-h-[calc(100vh-4rem)]">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-2xl font-bold text-[#1E3A8A]">
              {activeTab === "availableBids" && "Available Bids"}
              {activeTab === "myOffers" && "My Offers"}
              {activeTab === "paymentStatus" && "Payment Status"}
            </h1>
